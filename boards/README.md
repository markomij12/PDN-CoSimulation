...
| In2.Cu | **GND plane** |
| Inner dielectric | 0.2 mm prepreg, εr = 4.5 (the PDN cavity, In1.Cu ↔ In2.Cu) |
| Core | 1.04 mm, εr = 4.5, F.Cu↔In1.Cu |
| Bottom prepreg | 0.2 mm, εr = 4.5, In2.Cu↔B.Cu |
...
`U1` is a dummy 2-pin load (pad 1 = VCC, pad 2 = GND). `C1` and `C2` sit next to U1; `C3` is farther out so the optimizer has a near/far placement choice. Move a VCC via in KiCad and re-run `python run_pipeline.py --board boards/pdn_test.kicad_pcb` to change the extracted S-parameters.
